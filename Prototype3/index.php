<?php
include_once './Gestions/gestionStagiaires.php';
include_once './Gestions/ville.php';
$stagiaireData = new GestionStagiaire();
$stagiaireDataArray = $stagiaireData->getStagiaireInfo();
$citiesNames = new Villes();
$getCitiesNames = $citiesNames->getCity();

$cities = new Villes();
$getCitiesNames = $cities->getCities();

foreach ($getCitiesNames as $cityValues) {
    $city = new Stagiaire();
    $city->setId($cityValues['Id']);
    $city->setCity($cityValues['Nom']);
}

$citiesTrainerCount = new GestionStagiaire();
$theCountResult = $citiesTrainerCount->countTrainner();

if (isset($_POST['confirm_Data'])) {

    $manage = new Stagiaire();
    $manage->setName($_POST['name']);
    $manage->setCity($_POST['city']);
...
<body>
    ...
    <body>
                ...
                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="city_selector" name="city" required>
                            ...
                            <?php
                            foreach ($getCitiesNames as $cityName) {
                                ?>
        ...
        $totalRows = count($stagiaireDataArray); // Total number of rows
        ...
        $rowsOnPage = array_slice($stagiaireDataArray, $startRow, $rowsPerPage);
        ?>
        <section class="mx-5">
            <div>
